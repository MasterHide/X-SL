...
show_menu() {
    ...
    # Menu with different colors for each section
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${red}0.${plain} Exit Script"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${blue}1.${plain} Install ${server}"
    echo -e "${blue}2.${plain} Update ${refresh}"
    echo -e "${blue}3.${plain} Update Interface ${settings}"
    echo -e "${blue}4.${plain} Legacy Version 🕰️"
    echo -e "${blue}5.${plain} Uninstall ${stop}"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${yellow}6.${plain} Reset Username, Password, & Secret Token ${key}"
    echo -e "${yellow}7.${plain} Reset Panel Web Base Path ${lock}"
    echo -e "${yellow}8.${plain} Reset Settings ${settings}"
    echo -e "${yellow}9.${plain} Change Port ${settings}"
    echo -e "${yellow}10.${plain} View Current Panel Info ${server}"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${purple}11.${plain} Start ${rocket}"
    echo -e "${purple}12.${plain} Stop ${stop}"
    echo -e "${purple}13.${plain} Restart ${refresh}"
    echo -e "${purple}14.${plain} Check Status ${server}"
    echo -e "${purple}15.${plain} Logs Management ${settings}"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${cyan}16.${plain} Enable Autostart ${rocket}"
    echo -e "${cyan}17.${plain} Disable Autostart ${stop}"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${green}18.${plain} SSL Certificate Management ${ssl}"
    echo -e "${green}19.${plain} Cloudflare SSL Certificate ${ssl}"
    echo -e "${green}20.${plain} IP Limit Management ${firewall}"
    echo -e "${green}21.${plain} Firewall Management ${firewall}"
    echo -e "${green}22.${plain} SSH Port Forwarding Management ${settings}"
    echo -e "${blue}──────────────────────────────────────────────${plain}"
    echo -e "${red}23.${plain} Enable BBR ${bbr}"
